  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'} transition-colors duration-300`}>
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="sticky top-0 z-10 backdrop-blur-xl bg-opacity-80 border-b border-gray-200 dark:border-gray-700 p-4">
        ...
        >
          {videos.map((video, index) => (
            <div key={video.id} className="h-screen snap-start relative flex">
              {/* Video Player */}
              ...
              {/* Video Info Sidebar */}
              <div className={`w-96 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} border-l ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} flex flex-col transition-colors duration-300`}>
                <div className="flex-1 p-6 overflow-y-auto">
                  {/* Creator Info */}
                  <div className="flex items-center space-x-3 mb-4">
                    <div className={`w-12 h-12 ${isDarkMode ? 'bg-gradient-to-br from-blue-500 to-purple-600' : 'bg-gradient-to-br from-blue-400 to-purple-500'} rounded-full flex items-center justify-center text-white font-bold`}>
                      {video.avatar}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        ...
                        {video.verified && <Verified className="w-4 h-4 text-blue-500" />}
                      </div>
                  ...
                  {/* Category */}
                  <div className="mb-6">
                    <span className={`inline-block px-3 py-1 rounded-lg text-xs font-medium ${isDarkMode ? 'bg-blue-900/30 text-blue-300' : 'bg-blue-100 text-blue-800'}`}>
                      {video.category}
                    </span>
                  </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        ...
                        >
                          ...
                          <span className="text-sm font-medium">{video.likes + (likedVideos.includes(video.id) ? 1 : 0)}</span>
                        </button>

                        <button className={`flex items-center space-x-2 px-4 py-2 rounded-lg ${isDarkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'} transition-colors`}>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        >
                          <Bookmark className={`w-5 h-5 ${savedVideos.includes(video.id) ? 'fill-current' : ''}`} />
                        </button>

                        <button className={`p-2 rounded-lg ${isDarkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'} transition-colors`}>
                  ...
                  <div className="space-y-4">
                    <h4 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Comments</h4>
                    
                    {/* Sample Comments */}
                    <div className="space-y-3">
                      <div className="flex space-x-3">
                        <div className={`w-8 h-8 ${isDarkMode ? 'bg-green-600' : 'bg-green-500'} rounded-full flex items-center justify-center text-white text-xs font-bold`}>
                          JD
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-1">
                            <span className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>John Doe</span>
                      ...
                      <div className="flex space-x-3">
                        <div className={`w-8 h-8 ${isDarkMode ? 'bg-purple-600' : 'bg-purple-500'} rounded-full flex items-center justify-center text-white text-xs font-bold`}>
                          SM
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-1">
                            <span className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Sarah Miller</span>
                            <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>5 hours ago</span>
                          </div>
                          <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    ...
                    <div className="flex space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                      <div className={`w-8 h-8 ${isDarkMode ? 'bg-blue-600' : 'bg-blue-500'} rounded-full flex items-center justify-center text-white text-xs font-bold`}>
                        SH
                      </div>
                      <div className="flex-1">
                        <input
                          ...
                          className={`w-full px-3 py-2 rounded-lg border ${isDarkMode ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' : 'bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-500'} focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm`}
                        />
                      </div>
                    </div>
                  </div>
