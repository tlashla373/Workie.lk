...
import { Search, MapPin, DollarSign, Clock, Building, Heart, ExternalLink, Filter, X, User, Calendar, Phone, Mail, Star, MessageCircle } from 'lucide-react';
    ...
    <div className="space-y-6 p-6">
      ...
      <div className={`rounded-xl p-6 space-y-4 ${isDarkMode ? 'bg-gray-700/30' : 'bg-gray-50'}`}>
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <select
            ...
            className={`w-full lg:w-48 px-2 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-transparent transition-all duration-200 ${isDarkMode ? 'bg-gray-700/50 border-gray-600/50 text-white' : 'bg-white border-gray-300 text-gray-900'}`}
          >
      ...
      <div className="space-y-4">
        {filteredJobs.map(job => (
          <div 
            key={job.id} 
            className={`rounded-xl p-6 hover:transition-all duration-200 border cursor-pointer ${isDarkMode ? 'bg-gray-700/30 hover:bg-gray-700/40 border-gray-600/30' : 'bg-white hover:bg-gray-50 border-gray-200'}`}
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-2">
                      <h3 className={`text-xl font-semibold hover:text-blue-400 transition-colors duration-200 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                        {job.title}
                      </h3>
                      <div className="flex items-center space-x-2 mt-2 lg:mt-0">
                        ...
                        <button 
                          className={`p-2 rounded-lg transition-all duration-200 ${isDarkMode ? 'text-gray-400 hover:text-blue-400 hover:bg-blue-500/10' : 'text-gray-500 hover:text-blue-600 hover:bg-blue-100'}`}
                          onClick={(e) => e.stopPropagation()}
                        >
                    ...
                    {/* Publisher Info */}
                    <div className={`flex items-center space-x-3 mb-3 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      ...
                      <div className="flex space-x-3">
                        <button 
                          className={`px-4 py-2 rounded-lg transition-all duration-200 ${isDarkMode ? 'bg-gray-600/50 text-gray-300 hover:bg-gray-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                          onClick={(e) => e.stopPropagation()}
                        >
                          Save for Later
                        </button>
